...
for (const [url, tag] of [['https://www.era-residence.com/','ref'], ['http://localhost:3000/','loc']]) {
  const p = await (await b.newContext({viewport:{width:+process.argv[2]||1440,height:+process.argv[3]||900},isMobile:(+process.argv[2]||1440)<600,hasTouch:(+process.argv[2]||1440)<600})).newPage();
...
fs.writeFileSync(`C:/Users/AI TEAM/Desktop/Harsh_Projects/Front_end_website/docs/geom-${process.argv[2]||'1440'}.json`, JSON.stringify(out,null,1));
